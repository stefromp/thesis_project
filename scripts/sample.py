...
def sample(
    ...
    # For the static graph mode, reload the saved adjacency so the model
    # architecture can be reconstructed before loading the state dict.
    adjacency = None
    if graph_params is not None and graph_params.get('enabled', False):
        if graph_params.get('mode', 'static') == 'static':
            adj_path = os.path.join(parent_dir, 'adj.pt')
            if os.path.exists(adj_path):
                adjacency = torch.load(adj_path, map_location='cpu')
